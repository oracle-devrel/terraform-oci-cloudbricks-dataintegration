...
data "oci_identity_compartments" "COMPARTMENTS" {
  ...
  filter {
    name   = "name"
    values = [var.integration_workspace_instance_compartment_name]
  }
}
...
data "oci_identity_compartments" "NWCOMPARTMENTS" {
  ...
  filter {
    name   = "name"
    values = [var.integration_workspace_network_compartment_name]
  }
}
...
locals {
  ...
  vcn_ocid = lookup(data.oci_core_vcns.VCN.virtual_networks[0], "id")
}